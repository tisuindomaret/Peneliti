...
export default function ReviewApplicationPage({ params }: { params: { id: string } }) {
  ...
  const handleAction = async (actionFn: ((id: string, payload: Record<string, unknown>, token: string) => Promise<unknown>), payload: Record<string, unknown>) => {
    try {
      const token = localStorage.getItem('token');
      if (!token) return;
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Application Details: {application.applicationNumber || 'Draft'}</h1>
        <span className="px-3 py-1 bg-gray-200 rounded capitalize">{application.status.replace('_', ' ')}</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div>
          ...
          <div className="bg-white p-4 border rounded space-y-2 mb-4">
            ...
            <p><strong>Field:</strong> {application.fieldTopic}</p>
            <p><strong>Location:</strong> {application.location}</p>
            <p><strong>PI:</strong> {application.principalInvestigator}</p>
            <p><strong>Objective:</strong> {application.objective}</p>
          </div>
            ...
            {application.documents?.length === 0 ? (
              ...
              <ul className="space-y-4">
                {application.documents?.map((doc: Record<string, unknown>) => (
                  <li key={doc.id as string} className="border-b pb-2 flex justify-between items-center">
                    <div>
                      <p className="font-medium">{(doc.requirement as Record<string, string>)?.name || 'Document'}</p>
                      <p className="text-sm text-gray-500">Status: {doc.reviewStatus as string}</p>
                      <a href={`/api/v1/files/${doc.fileId}`} target="_blank" rel="noreferrer" className="text-blue-600 text-sm hover:underline">
                        View File
                      </a>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => handleAction(reviewApplication, { documentReviews: [{ documentId: doc.id, status: 'accepted' }] })}
                        className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded"
                      >
                        Accept
                      </button>
                      <button
                        onClick={() => handleAction(reviewApplication, { documentReviews: [{ documentId: doc.id, status: 'needs_revision' }] })}
              ...
              <ul className="space-y-3">
                {history.map((h) => (
                  <li key={h.id as string} className="border-b pb-2">
                    <p className="text-sm text-gray-500">{new Date(h.createdAt as string).toLocaleString()}</p>
                    <p className="font-medium">{h.fromStatus as string} &rarr; {h.toStatus as string}</p>
                    {Boolean(h.note) && <p className="text-gray-700 italic text-sm">{h.note as string}</p>}
                  </li>
                ))}
              </ul>
            )}
      ...
      <div className="mt-8">
        ...
        <div className="flex flex-wrap gap-4 bg-white p-4 border rounded">
          ...
          <div className="w-full mt-4 pt-4 border-t">
            ...
            <button
              onClick={() => {
                const list = actionList.split(',').map(s => ({ field: 'general', instruction: s.trim() })).filter(a => a.instruction);
                handleAction(requestRevision, { reason: reason || 'Revision required', actionList: list });
              }}
